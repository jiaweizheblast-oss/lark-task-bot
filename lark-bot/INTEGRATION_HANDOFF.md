...
## 2026-07-21 Channel Analytics v2 统一流程

Channel Analytics 的主数据集是来源中立的 **Candidate Pipeline**：每行一个候选人，
无论候选人来自 Talent Discovery、LinkedIn、Naukri、Telegram、WhatsApp、员工推荐、
招聘会、代理或其他渠道，都进入同一条招聘阶段流水。Talent Discovery 只是受控来源之一，
不是特殊层级，也不会成为 Nexus 候选人域的唯一入口。
...
- Excel 使用隐藏 `Row Ref`，Lark 使用 `System ID`/记录 ID 做身份和幂等；绝不按姓名、URL、
...
- Task Bot 保留原 `APP_ID` / `APP_SECRET`，继续负责任务管理。
- Talent Discovery 与 Channel Analytics 统一由 RECRUITMENT BOT 对外服务。
- Talent Discovery 本地长连接是 RECRUITMENT BOT 的唯一 Lark 消息消费者；Railway 不与它争抢招聘事件。
- Railway 仅使用独立的 `RECRUITMENT_LARK_APP_ID` / `RECRUITMENT_LARK_APP_SECRET` 读写 Channel Base。
...
| 变量 | 说明 |
|---|---|
| `APP_ID` / `APP_SECRET` | Task Bot 应用凭证。**不要替换成 Recruitment Bot，也不要进 GitHub**。 |
| `RECRUITMENT_LARK_APP_ID` / `RECRUITMENT_LARK_APP_SECRET` | Recruitment Bot 应用凭证，只供 Channel Analytics Base API 使用。 |
| `RECRUITMENT_BOT_NAME` | 固定为 `RECRUITMENT BOT`，用于网站显示正确应用身份。 |
...
- **`candidate`（本次重点，候选人级招聘跟踪）**：
  `id, apply_date, name, channel, source_detail, job_request_id(FK job_requests, 可空), status, note, filled_by, source, ext_ref, lark_record_id, created_at, updated_at`
  - `status` 使用上方 v2 十阶段词表，可从任意阶段直接建档。
  - **`ext_ref`** = 预留给 **CODEX 候选人的不透明关联号**（两库不合并，只存对方 token）。
  - **`lark_record_id`** = 对应 Lark 多维表格记录号，导入去重用。
- `candidate_stage_event` —— 追加式阶段历史；`event_ref` 唯一，重复提交不会重复建事件。
- `channel_daily` —— 尚未逐人建档的人工渠道数量，唯一键包含日期、来源、来源详情和职位。
...
- **Candidate Pipeline Excel**：`/api/channel/template`（下载）、`/api/channel/upload`（上传，`.xlsx` only）。
- **阶段历史**：`/api/candidates/<id>/stage-events`。
- **Lark 同步**：`/api/lark/ping`（高级设置中的连接自检）、`/status`、`/table`（创建 v2 Base）、`/pull`（读取两张表）。旧 push 入口 fail-closed。
...
- **数据模型**：**每行一个可识别候选人**（`candidate`），阶段变更进入 `candidate_stage_event`。
- **来源模型**：来源必须来自受控词表；`Other` 必须有详情。既有招聘中的人可以从面试、Offer、Hired、Rejected 等阶段直接录入，无需伪造早期阶段。
- **两条等价录入路**：网站 `.xlsx` 与 Lark `Candidate Pipeline` 表使用相同字段和服务；网站不是 Lark 多维表格文件下载，而是可离线填写的 Excel，但业务语义相同。
- **身份与幂等**：网站按隐藏 `Row Ref`，Lark 按记录 ID/`System ID`；已知行更新、稳定引用重传 no-op/更正，未知或冲突引用拒绝。禁止按姓名猜身份。
- **分析口径**：逐人建档统计从候选人和阶段历史派生；未识别人头的人工数量单独展示，不与派生人数合并。

---

## 7. Lark 集成现状

- `lark_bitable.py`：RECRUITMENT BOT 以**应用身份**（tenant_access_token，用独立的 `RECRUITMENT_LARK_APP_ID/RECRUITMENT_LARK_APP_SECRET`）建/读/写自己的 Channel Base。Task Bot 凭证不会用于招聘表。
- **已联调**：`① 连接自检`（`/api/lark/ping`）线上通过 —— 机器人能拿到 Lark token（app 尾号 `78deea`）。
- **待联调**：创建 `channel-analytics-v2` Base，并用一条安全测试记录验证 Bot 获取/提交、网站刷新和重复提交幂等；本地测试不等于真实 Lark 已验证。
...
- ✅ **本地验证通过**：统一 Candidate Pipeline、受控来源与 `Other` 详情、任意阶段进入、阶段历史、Excel 幂等、Lark v2 服务、旧 push fail-closed。
- ⏳ **部署后待联调**：创建新的 v2 Lark Base，验证 Bot 获取/提交和网站同步。旧 v1 配置不会被当成 v2 静默复用。
