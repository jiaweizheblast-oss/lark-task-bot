        ...
        with conn.cursor(
            ...
            if existing:
                if existing["status"] == "failed":
                    # The worker may have created a local/Lark artifact before
                    # Railway received the receipt.  Retrying the same revision
                    # can therefore collide with that artifact.  Only the
                    # explicit repair endpoint may archive this command and
                    # queue revision + 1.
                    raise ValueError(
                        "failed daily publication must be repaired as a new revision"
                    )
...
def replace_published_talent_daily_publication(
    ...
    A failed command may already have created a local/Lark artifact before its
    success receipt reached Railway.  Replacing that immutable command with a
    new revision lets the local worker archive the unused partial artifact and
    publish the same frozen cohort again without running the scanner.
    """
    ...
    try:
        with conn.cursor(
            ...
            if current["status"] not in {"published", "failed"}:
                raise ValueError(
                    "only a published or failed recruiting table can be replaced"
                )
