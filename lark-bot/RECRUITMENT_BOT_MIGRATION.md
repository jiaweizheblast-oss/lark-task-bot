# Task Bot 与 RECRUITMENT BOT 安全分离

目标：同一个 Railway 网站保留两个独立 Lark 应用。`Task Bot` 继续处理任务管理；Talent Discovery 与 Channel Analytics 统一使用 `RECRUITMENT BOT`。

## 安全边界

- 不删除候选人、职位、任务、招聘阶段历史或 ContactActivity。
- 原有 `APP_ID` / `APP_SECRET` / `BOT_NAME` 永远保留给 Task Bot。
- Channel Analytics 只读取新增的 `RECRUITMENT_LARK_APP_ID` / `RECRUITMENT_LARK_APP_SECRET` / `RECRUITMENT_BOT_NAME`。
...
5. Railway Variables 中**不要修改**现有 `APP_ID`、`APP_SECRET`、`BOT_NAME`。新增：
   - `RECRUITMENT_LARK_APP_ID` = 本地 AI-Talent-Discovery `.env` 中 RECRUITMENT BOT 的 `LARK_APP_ID`
   - `RECRUITMENT_LARK_APP_SECRET` = 同一应用的 `LARK_APP_SECRET`
   - `RECRUITMENT_BOT_NAME` = `RECRUITMENT BOT`
...
- Task Bot 继续正常处理任务管理，但渠道命令会提示改用 RECRUITMENT BOT。
- RECRUITMENT BOT 是 Talent Discovery 与 Channel Analytics 的唯一入口。
